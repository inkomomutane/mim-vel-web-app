        ...
        <DropdownMenuContent align="end" class="max-w-[600px]">
            <template v-for="(action, index) in props.actions" :key="index">
                <DropdownMenuItem  :disabled="action?.disabled || false"
                                   @click="action?.disabled  ? () => {} : action.onClick(props.row)"
                ...
                <DropdownMenuSub v-if="action.children && action.children.length > 0">
                    ...
                    <DropdownMenuSubContent align="start" class="w-[160px]">
                        <template v-for="(childAction, childIndex) in action.children" :key="childIndex">
                            <DropdownMenuItem  :disabled="childAction?.disabled || false"  @click="childAction?.disabled  ? () => {} : childAction.onClick(props.row)">
