...
function computeScrollTopForElement({
    ...
    if (align === "center") {
        ...
        return (
            offsetTop - padding.start - (available - height) / 2 - scrollMargin
        );
    }
    if (align === "end")
        return (
...
function computeVisibility({
    ...
    for (const child of getMessageChildren(content, spacer)) {
        ...
        const rect =
            isAnchor || noIntersectionObserver
...
function createEngine(props: MessageScrollerProviderProps) {
    ...
    function flushPendingScrollToMessage(): boolean {
        ...
        if (
            !element ||
            !scrollToElement(element, pendingScrollToMessage.options)
        )
            return false;
        pendingScrollToMessage = null;
        defaultScrollPositionApplied = true;
    ...
    function applyContentChange(
        ...
        if (children.length === previousCount) {
            const anchor = findFirstUnhandledAnchor(
                children,
                handledScrollAnchors,
            );
            if (anchor) {
                scrollToElement(
                    anchor,
    ...
    const registerMessage: RegisterMessage = (
        ...
        if (
            previousElement &&
            messageElements.get(messageId) === previousElement
        ) {
