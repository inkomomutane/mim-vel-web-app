...
const labelledBy = computed(
    () =>
        [...titleIds.value, attrs["aria-labelledby"]]
            .filter(Boolean)
            .join(" ") || undefined,
);
...
const keyShortcuts = computed(
    () =>
        [
            ...
            active.value && answers.value.length
                ? "ArrowUp ArrowDown"
                : undefined,
...
function registerAnswerSelection(answerId: string, defaultSelected: boolean) {
    if (defaultSelected) {
        defaultSelectedAnswerIds = [
            ...defaultSelectedAnswerIds.filter(
                (current) => current !== answerId,
            ),
            answerId,
        ];
...
function registerTitle(titleId: string) {
    ...
    return () => {
        titleIds.value = titleIds.value.filter(
            (current) => current !== titleId,
        );
    };
}
...
function registerError(errorId: string) {
    ...
    return () => {
        errorIds.value = errorIds.value.filter(
            (current) => current !== errorId,
        );
    };
}
...
function getAnswerByShortcut(shortcut: string) {
    if (shortcutByChoiceValue.value) {
        const choiceValue = Array.from(
            shortcutByChoiceValue.value.entries(),
        ).find(([, choiceShortcut]) => choiceShortcut === shortcut)?.[0];

        return (
            answers.value.find(
                (answer) =>
                    answer.type === "choice" && answer.value === choiceValue,
            ) ?? null
        );
    }
...
<template>
    <fieldset
        ...
        :class="
            cn(
                'flex flex-col gap-5 min-w-0 border-0 p-0 outline-none',
                props.class,
            )
        "
    >
