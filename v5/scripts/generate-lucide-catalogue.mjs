...
const outputPath = path.resolve("resources/data/lucide-icons.json");
...
function createSearchText({ name, slug, source, tags = [] }) {
    return [name, slug, source, ...tags].join(" ").toLowerCase();
}
...
const lucide = Object.entries(lucideNodes).map(([slug, iconNode]) => {
    const tags = Array.isArray(lucideTags[slug]) ? lucideTags[slug] : [];
...
const icons = [...lucide, ...lab].sort((a, b) => {
...
await fs.writeFile(outputPath, JSON.stringify(icons, null, 2));

console.log(`Generated ${icons.length} icons`);

console.log(`Lucide: ${lucide.length}`);

console.log(`Lab: ${lab.length}`);
