...
class GetImovels
{
    ...
    public function handle(?string $term = null, User $user)
    {
        if ($user->hasAnyRole('Super-Admin', 'Admin')) {
            ...
            return ImovelData::collection(
                Imovel::query()
                    ...
                    },])->orderBy('updated_at', 'desc')->paginate(5)->withQueryString()
            );
        } else {
            /** @var Collection<Imovel> $imovels */
            $imovels = Imovel::query()->when($term, function ($query, $search) {
                $query->where('titulo', 'like', '%' . $search . '%')
                    ->orWhereRelation('bairro', 'nome', 'like', '%' . $search . '%')
                    ->orWhereRelation('bairro.cidade', 'nome', 'like', '%' . $search . '%')
                    ->orWhereRelation('bairro.cidade.province', 'name', 'like', '%' . $search . '%');
            ...
            })->with([ 'corretor','regraDeNegocio','intermediationRule' ,'bairro.cidade.province', 'media' => function (MorphMany $query) {
                $query->where('collection_name', 'posts');
