        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4>Tabela de status</h4>
                    <div class="card-header-action">
                        <button class="btn btn-info" data-toggle="modal" data-target="#novoStatus"><i
                                class="fas fa-plus"></i><span> Novo status</span></button>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped v_center" id="statuses">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                @foreach ($statuses as $status)
                                    <tr>
                                        ...
                                        <td>{{ ucfirst($status->id) }}</td>
                                        <td>{{ ucfirst($status->nome) }}</td>
                                        <td><button class="btn btn-secondary" data-toggle="modal" data-target="#EditStatus"
                                                data-code="{{ ucfirst($status) }}"><i class="fa fa-edit"></i></button>
                                        </td>
                                        <td><a href="#" class="btn btn-danger" data-toggle="modal"
                                                data-target="#DeleteStatus" data-code="{{ ucfirst($status) }}"><i
...
@section('modals')
                        ...
                        <div class="container form-group row">
                            ...
                            <div class="col-md-12">
                                <input id="nome" type="text" class="form-control" name="nome"
                                    placeholder="Digite nome de status" required autocomplete="nome" autofocus>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="EditStatus">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('status.index') }}" method="post" id="edit_form">
                    @csrf
                    @method('PATCH')
                    <div class="modal-body">
                        <div class="container form-group row">
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="DeleteStatus">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">Deletar status</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('status.index') }}" method="post" id="delete_form">
                    @csrf
                    @method('DELETE')
                    <div class="modal-footer bg-whitesmoke br">
    ...
    <script>
        ...
        $(function() {
            $('#EditStatus').on('show.bs.modal', function(event) {
                ...
                form.attr('action', '{!! route('status.index') !!}' + '/' + code.id);
